    ...
    return (
        <section className={styles.section}>
            ...
            <List sx={{ width: '100%', bgcolor: "#f7f9fa", border: 1, borderColor: "#e8eaec" }} disablePadding component="nav">
                {content.sections.filter(item => !showmore ? item.index <= 10 : true).map( item => <ContentSection key={item.index} item={item} expand={expand}/>)}
            </List>
            { !showmore ? <button onClick={handleButton} className={styles.showmore}>{content.sections.length - 10} more sections</button> : ''}
        </section>
    )
}
