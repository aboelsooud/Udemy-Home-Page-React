...
function ContentSection({item, expand}) {
    ...
    return (
        <>
            <ListItemButton sx={{ width: '100%', bgcolor: "#f7f9fa", border: 1, borderColor: "#e8eaec", display: 'flex'}} onClick={updateOpen} >
                <div className={styles.left}>
                    {open ? <ExpandLess className={styles.leftData}/> : <ExpandMore className={styles.leftData}/>}
            ...
            <Collapse in={open} timeout="auto" unmountOnExit>
                <List component="div" sx={{bgcolor: "white", padding: 2}}>
                    {item.items.map(lec => <ContentLecture key={lec.id} lec={lec}/>)}
                </List>
            </Collapse>
        </>
    )
